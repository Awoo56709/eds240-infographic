...
1. Key feedback points I received on my FPM #3 draft visualizations
Across peer review and instructor feedback, several themes emerged:

Clarity of annotations: Some labels overlapped or competed with the data, especially in the 30‑minute plot and the complexity plot. Reviewers suggested repositioning annotations and improving hierarchy.

Axis readability: A few plots had axis labels or tick marks that were either too small or too dense.

Complexity plot labeling: Multiple reviewers suggested finding a more creative way to label the rating tiers instead of relying on a traditional x‑axis.

Ingredient sunburst: Feedback encouraged simplifying ingredient categories and ensuring the hierarchy was intuitive.

Nutrition plot: Reviewers recommended choosing a single nutritional variable and/or faceting by category to avoid clutter and improve interpretability.

Overall cohesion: Several comments encouraged me to lean further into my editorial “menu” metaphor and unify the aesthetic across all plots (color, typography, spacing).

2. Which suggestions I implemented and how
I incorporated nearly all major suggestions:

Improved annotation placement:  
I repositioned labels in the 30‑minute plot (e.g., moving the recipe count label downward, adjusting the 30‑minute marker, and refining the curved arrow). This improved legibility and visual flow.

Refined axis readability:  
I increased caption size, adjusted tick spacing, and used log‑scaled review counts with intuitive labels (“1”, “10”, “100”, “1k”, “10k”).

Creative labeling for the complexity plot:  
Instead of traditional x‑axis labels, I added left‑aligned text annotations beside each stacked bar. This created a cleaner, more editorial look and aligned with feedback to experiment with non‑traditional labeling.

Ingredient sunburst simplification:  
I filtered out generic ingredients (salt, pepper, water, oil, butter) and limited each category to its top five ingredients. This made the sunburst more interpretable and visually balanced.

Nutrition plot redesign:  
Based on feedback, I selected sugar as the single nutritional variable and faceted the scatterplot by the three main categories (Dessert, Main Dish, Side Dish). This clarified the relationship between nutrition and popularity and avoided overplotting.

Cohesive aesthetic:  
I standardized my palette (warm terracotta, caramel, cream), typography (Playfair Display + Lato), and background tone (#F5EFE6 / #F7F2EB). This created a unified editorial feel across all visualizations.

3. Feedback I chose not to implement and why
Adding more categories to the nutrition facet:  
Some reviewers suggested including all recipe categories. I decided against this because it diluted the clarity of the comparison and made the layout visually overwhelming. Focusing on the three main categories preserved readability and narrative focus.

Adding more complexity tiers:  
A suggestion was made to break complexity into more granular bins. I kept the three‑tier system (Simple, Moderate, Complex) because it aligned better with my “layered cake” metaphor and maintained interpretability.

Using a different chart type for the 30‑minute plot:  
A few peers suggested a histogram or line plot. I kept the area chart because it visually reinforces the “mountain peak” of engagement at 30 minutes and fits the editorial tone.

4. Ten design elements I did not fully implement (and why)
I addressed nearly all ten elements, but a few were less central to my project:

DEI lens:  
The dataset (Food.com recipes) does not directly represent people or communities in a way that raises equity concerns. I still considered cultural categories carefully, but DEI was not a major design driver.

Accessibility through colorblind palettes:  
My palette is warm and editorial, and while contrast is strong, it is not strictly colorblind‑optimized. I prioritized aesthetic cohesion for this specific audience (general readers of a food‑magazine‑style infographic).

Contextualizing uncertainty:  
Because my visualizations focus on descriptive patterns rather than inferential statistics, uncertainty intervals were not central to the story. I used LOESS smoothing where appropriate but did not emphasize statistical uncertainty.

Everything else 
— hierarchy, typography, color, annotation, spacing, narrative flow 
— was implemented intentionally and consistently.

# What Makes a Recipe Popular? A Data‑Driven Look at How We Cook at Home

Why do some recipes take off while others barely get noticed?
It’s a question every home cook, food blogger, and recipe platform has wondered about — and one that feels surprisingly personal. We all have dishes we return to again and again, and others we scroll past without a second thought. But what actually drives that difference at scale?

To explore this, I turned to a massive dataset of more than 188,000 recipes published on Food.com  between 1999 and 2016. With rich metadata on ingredients, cooking times, ratings, and nutritional information, the dataset offered a rare opportunity to examine the patterns behind recipe engagement.

My goal was simple:
understand what makes a recipe resonate with home cooks.
...
# Final Infographic
(Insert the PNG or rendered plot here — centered and sized appropriately.)

# Designing the Infographic: A Behind‑the‑Scenes Look
...
I structured the infographic like a meal: a starter, a main dish, a side, and a dessert.
...
- The ingredient sunburst and sugar‑popularity scatterplot act as side dishes that deepen the story.

This structure guides the viewer through the infographic the way a menu guides a diner.
...
- maintain cohesion across all visualizations

The palette also reinforces the editorial tone I wanted.

## 3. Typography
I paired Playfair Display for titles with Lato for body text and annotations.
This serif‑sans pairing is common in modern food media and strikes a balance between elegance and readability.
...
## 5. Layout
I used generous spacing, centered titles, and consistent margins to maintain an airy, magazine‑style feel. Circular elements (like the sunburst and radial calorie plot) balance the rectangular bar charts and scatterplots, creating visual variety without clutter.
...
- Ingredients → category identity

- Sugar → popularity

I intentionally avoided overplotting and limited categories where necessary (e.g., top five ingredients per category, three main facets for the sugar plot).

## 7. Consistency
All plots share:

- the same background tone (#F5EFE6 / #F7F2EB)

- the same font system

- similar annotation styling

- a warm, cohesive palette

This consistency makes the infographic feel like a unified piece rather than a collection of separate charts.
...
- Nutrition — specifically sugar — plays a subtle but category‑dependent role in popularity.
...
The dataset does not directly represent people or communities in a way that raises equity concerns, but I was mindful of how I categorized cuisines and avoided reinforcing stereotypes. I focused on descriptive patterns rather than value judgments.
...
Popularity isn’t random. It reflects how people actually cook.
...
In the end, this infographic is both a data story and a culinary one — a look at the rhythms, preferences, and habits that shape how we cook and eat at home.
...
recipes_clean <- recipes_clean |>
  ...
  filter(
    sugar_content >= 0,
    sugar_content <= 500   # optional cap for sanity
  )

# recipes_clean <- recipes_clean |>
#   mutate(
#     sugar_content = readr::parse_number(sugar_content)
#   ) |>
#   filter(
#     sugar_content >= 0,
#     sugar_content <= 500
...
# Visual 1: The 30-Minute Sweet Spot
```{r}
#| eval: true
#| echo: false
#| warning: false
...
# Visual 2: Recipe Complexity by Rating Tier
...
# --- Data prep (unchanged except spelling fix) ---
df <- recipes_clean |>
  mutate(
...
p <- ggplot(df, aes(x = rating_cat, y = prop, fill = complexity_cat)) +
  geom_col(
    color = "white",
    linewidth = 1,
    width = 0.7   # ← adds spacing between bars
  ) +
...
# Convert factor positions to numeric for annotation placement
rating_positions <- as.numeric(levels(df$rating_cat))

p +
  annotate(
    "text",
    x = rating_positions - 0.55,   # ← shifts labels left of bars
...
#Visual 3: Calories Over Time (Radial)
...
nutri_df <- recipes_clean |>
  ...
  mutate(
    review_log = log10(review_count),      # log10(1) = 0 → clean axis start
    sugar_trim = pmin(sugar_content, 200)  # trims extreme outliers for readability
  )

ggplot(nutri_df, aes(x = review_log, y = sugar_trim)) +
  geom_point(
    alpha = 0.56,
    color = "#E8CFA9",   # champagne bubble gold
    size = 2
  ) +
  geom_smooth(
    method = "loess",
    se = FALSE,
    color = "#C49A6C",   # warm bronze
    linewidth = 1.2
  ) +
...
# showtext_auto()
# 
# # Extract top 5 ingredients per category
...
# level2 <- top_ing5 |>
#   mutate(
#     id = paste(category_group, ingredient, sep = "-"),
...
#   )
# 
# sunburst_all <- bind_rows(level1, level2)
# 
# # Plot
# plot_ly(
#   labels = sunburst_all$label,
#   parents = sunburst_all$parent,
#   values = sunburst_all$value,
#   type = "sunburst",
#   branchvalues = "total",
#   maxdepth = 2
# )
...
ggplot() +
  ...
  "Main Dish" = "#D97A40",   # roasted orange
  "Dessert"   = "#E8A6C1",   # berry pink
  "Side Dish" = "#A3C586",   # herb green
  "Soup"      = "#E2C275",   # golden broth
  "Breakfast" = "#F2D8A7",   # oat/cream
  "Bread"     = "#C9A27E"    # whole wheat
)) +
  labs(
...
#| eval: true
#| echo: false
top_by_category <- recipes_clean |>
  group_by(category_group) |>
  slice_max(review_count, n = 5, with_ties = FALSE) |>
  arrange(category_group, desc(review_count))
...
#| eval: true
#| echo: false
lowest_by_category <- recipes_clean |>
  filter(review_count > 0) |>
  group_by(category_group) |>
  slice_min(review_count, n = 5, with_ties = FALSE) |>
  arrange(category_group, review_count)
